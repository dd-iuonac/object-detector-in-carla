    ...
    def evaluate(self):
        while True:
            ...
            if not cur_model_path:
                ...
                self._log_string('Found already evaluated checkpoint. Will try again in %d seconds' %
                                 self.eval_interval_secs)
            else:  # evaluating
                self.last_eval_model_path = cur_model_path

                with tf.Session() as sess:
                    self._log_string('**** Evaluation New Result ****')
                    self._log_string('Assign From checkpoint: %s' % self.last_eval_model_path)

                    self.saver.restore(sess, self.last_eval_model_path)
                    result_list = self.dataset.evaluation(sess, self.feedDictProducer, self.pred_list, self.val_size,
