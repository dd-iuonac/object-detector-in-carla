...
from core.trainer_utils import get_variables_in_checkpoint_file, get_bn_decay, get_learning_rate, average_gradients, \
    get_trainable_loss, get_trainable_parameter


def parse_args():
    parser = argparse.ArgumentParser(description='Trainer')
    parser.add_argument('--cfg', required=True, help='Config file for training')
    parser.add_argument('--img_list', default='train', help='Train/Val/Train val list')
...
class Trainer:
    def __init__(self, args):
        ...
        # feed dict
        self.feedDictProducer = FeedDictCreater(self.dataset_iter, self.model_list, self.batch_size)

        with tf.device('/gpu:0'):
            self.grads = average_gradients(self.tower_grads)
            self.update_op = [
                self.optimizer.apply_gradients(zip(self.grads, self.params), global_step=self.global_step)]
        self.update_op.extend(self.extra_update_ops)
        self.train_op = tf.group(*self.update_op)

        # tensor flow training ops
    ...
    def _initialize_model(self):
        ...
        if self.restore_model_path is not None:
            ...
            saver.restore(self.sess, path.split(".meta")[0])
    ...
    def train(self):
        ...
        for step in range(self.max_iteration):
            ...
            feed_dict = self.feedDictProducer.create_feed_dict()
