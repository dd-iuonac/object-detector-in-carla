...
def hard_sample_miner(cls_loss, loc_loss, pmask, nmask, pred_box, iou_thresh=0.7, num_hard_sample=128, loss_type='both',
                      cls_loss_weights=0.05, loc_loss_weights=0.06, max_negatives_per_positive=None,
                      min_negatives_per_image=16):
    if loss_type is 'loc':
        image_loss = loc_loss
    ...
    if max_negatives_per_positive is not None:
        nms_index = subsample_to_desire_ratio(nms_index, pmask, nmask, max_negatives_per_positive,
...
def calc_square_dist(a, b, norm=True):
    ...
    if norm:
        dist = tf.sqrt(a_square + b_square - 2 * tf.matmul(a, tf.transpose(b, [0, 2, 1]))) / tf.cast(tf.shape(a)[-1],
                                                                                                     tf.float32)  # [bs, npoint, ndataset]
    else:
