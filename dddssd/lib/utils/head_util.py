...
import dddssd.lib.utils.tf_util as tf_util
...
def box_regression_head(feature_input, pred_cls_channel, pred_reg_base_num, pred_reg_channel_num, bn, is_training,
                        pred_attr_velo, conv_op, bn_decay, output_dict):
    """
    ...
    pred_reg = conv_op(pred_reg, pred_reg_base_num * (pred_reg_channel_num + cfg.MODEL.ANGLE_CLS_NUM * 2),
                       activation_fn=None, scope='pred_reg')
    pred_reg = tf.reshape(pred_reg,
                          [bs, points_num, pred_reg_base_num, pred_reg_channel_num + cfg.MODEL.ANGLE_CLS_NUM * 2])

    if pred_attr_velo:  # velocity and attribute
        pred_attr = conv_op(feature_input, 128, bn=bn, is_training=is_training, scope='pred_attr_base',
                            bn_decay=bn_decay)
        ...
        pred_velo = conv_op(feature_input, 128, bn=bn, is_training=is_training, scope='pred_velo_base',
                            bn_decay=bn_decay)
    ...
    output_dict[maps_dict.PRED_ANGLE_CLS].append(tf.slice(pred_reg, \
                                                          [0, 0, 0, pred_reg_channel_num],
                                                          [-1, -1, -1, cfg.MODEL.ANGLE_CLS_NUM]))
    output_dict[maps_dict.PRED_ANGLE_RES].append(tf.slice(pred_reg, \
                                                          [0, 0, 0, pred_reg_channel_num + cfg.MODEL.ANGLE_CLS_NUM],
                                                          [-1, -1, -1, -1]))
