...
from dddssd.lib.builder.anchor_builder import Anchors
from dddssd.lib.builder.target_assigner import TargetAssigner
from dddssd.lib.builder.layer_builder import LayerBuilder
from dddssd.lib.dataset.placeholders import PlaceHolders
from dddssd.lib.modeling.head_builder import HeadBuilder
from dddssd.lib.builder.encoder_builder import EncoderDecoder
from dddssd.lib.builder.postprocessor import PostProcessor
from dddssd.lib.builder.loss_builder import LossBuilder

from dddssd.lib.utils.box_3d_utils import transfer_box3d_to_corners
from dddssd.lib.utils.model_util import *
...
class SingleStageDetector:
    ...
    def network_forward(self, point_cloud, bn_decay):
        ...
        for layer in self.layers:
            xyz_list, feature_list, fps_idx_list = \
                layer.build_layer(xyz_list, feature_list, fps_idx_list, bn_decay, self.output)
    ...
    def train_forward(self, index, anchors):
        ...
        returned_list = self.target_assigner.assign(base_xyz, anchors, gt_boxes_3d, gt_classes, gt_angle_cls,
                                                    gt_angle_res, gt_velocity, gt_attributes)

        assigned_idx, assigned_pmask, assigned_nmask, assigned_gt_boxes_3d, assigned_gt_labels, assigned_gt_angle_cls, \
        assigned_gt_angle_res, assigned_gt_velocity, assigned_gt_attribute = returned_list
        ...
        # corner_loss
        corner_loss_angle_cls = tf.cast(tf.one_hot(assigned_gt_angle_cls, depth=cfg.MODEL.ANGLE_CLS_NUM, on_value=1,
                                                   off_value=0, axis=-1), tf.float32) # bs, pts_num, cls_num, -1
        ...
        self.loss_builder.forward(index, self.labels, self.output, self.placeholders, self.corner_loss, self.vote_loss,
                                  self.attr_velo_loss, self.iou_loss)
