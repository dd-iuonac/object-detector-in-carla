...
class InfThread(Thread):
    ...
    def run(self):
        while self._running:
            m = self.message.get(block=True)
            a = self.evaluator.evaluate(m)
            # a = self.evaluator.infer(m)
            self.answer.put(a, block=False)
            print(a)

    def quit(self):
        self._running = False
        self.evaluator.session.close()
